...
struct vectorHash{
    int operator()(vector<vector<char>> const &v) const {
        long long curr = 1;
        for (int i = 0; i < v.size(); i++) {
            for(int j = 0; j < v[i].size(); j++) {
                curr *= (int) (v[i][j] + i*j + j);
                curr %= 1000000000;
            }
        }
        return curr;
...
unordered_map<vector<vector<char>>, vector<pair<int, pair<int, int>>>, vectorHash> posMoves;
...
struct position
{
    ...
    int cornerMobility2() {
        ...
        double mobility = 2000 * (whiteMoves - blackMoves) / (whiteMoves + blackMoves);
        ...
        for (int i = 0; i < 7; i++) {
            ...
            for (int j = 0; j < 7; j++) {
                if (board[i][j] == 'W') tempWhite++;
                    ...
                    tempWhite = tempBlack = 0;
                    break;
                }
            }
            whiteStable += 300 * tempWhite;
            blackStable += 300 * tempBlack;
        }
        for (int i = 0; i < 7; i++) {
            int tempWhite = 0;
            int tempBlack = 0;
    ...
    int evaluate2() {
        return frontierStable() + cornerMobility2() + parity() + internalStability2();
    }
...
struct engine
{
    ...
    int bestEvalDepth(int depth, int layers, int alpha, int beta, position thisPos) {
        ...
        if (thisPos.endOfGame()){
            if (layers) return thisPos.evaluate2();
            return 0;
        }
    ...
    int bestEvalDepth1(int depth, int layers, int alpha, int beta, position thisPos) {
        ...
        if (thisPos.endOfGame()){
            if (layers) return thisPos.evaluate1();
            return 0;
        }
        ...
        if (thisPos.toPlay == 'W') {
            ...
            for (int i = 0; i < legalMoves.size(); i++) {
                position tempPos = position(thisPos.currBoard, thisPos.toPlay, thisPos.pieceCounts[1], thisPos.pieceCounts[0], thisPos.skippableMoves);
                tempPos.makeMove(legalMoves[i].first, legalMoves[i].second);
                int tempEval = bestEvalDepth1(depth-1, layers+1, alpha, beta, tempPos);
                //cout << "Move " << legalMoves[i].first << " " << legalMoves[i].second << " evaluated at " << tempEval << "\n";
                if (tempEval > bestEval) {
                   bestEval = tempEval;
                   bestInd = i;
